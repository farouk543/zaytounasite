...
namespace App\Filament\Resources\SummerClubQuizAttempts\Tables;
...
class SummerClubQuizAttemptsTable
{
    public static function configure(Table $table): Table
    {
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('quiz.subject')
                    ->label('Matiere')
                    ->formatStateUsing(fn ($state, $record) => $state ?: ($record->quiz?->resource?->subject ?: '-'))
                    ->placeholder('-')
                    ->sortable(),

                Tables\Columns\TextColumn::make('quiz.level')
                    ->label('Niveau')
                    ->formatStateUsing(function (?string $state, $record) {
                        $level = $state ?: $record->quiz?->resource?->level;
            ...
            ->filters([
                ...
                SelectFilter::make('subject')
                    ...
                    ->query(fn (Builder $query, array $data) => filled($data['value'] ?? null)
                        ? $query->whereHas('quiz', fn (Builder $query) => $query->where(function (Builder $query) use ($data) {
