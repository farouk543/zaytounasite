      ...
      <div class="lux-card" style="padding:22px;" data-reveal>
        ...
        <div style="margin-top:16px; display:grid; gap:10px; color:#0f172a;">
          ...
          @else
            ...
            <div><strong>{{ __('ui.checkout.hour_price') }}</strong> {{ $pricing['hour_price'] ?? 22 }} {{ $symbol }} / {{ __('ui.checkout.per_hour') }}</div>
          @endif
        </div>
        ...
        @if(!$isCustom)
          <div style="display:flex; justify-content:space-between; gap:14px; flex-wrap:wrap; align-items:flex-end;">
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">
              <a class="btn-outline" href="{{ route('regimes.france.show') }}">{{ __('ui.back') }}</a>
              <form method="POST" action="{{ route('regimes.france.submit') }}" style="display:inline;">
                @csrf
