...
@php
  ...
  $levelName = null; $subjectName = null;
  ...
  $backUrl = $backRoute ?? route('regimes.saudi.show');
@endphp
...
<section class="za-section za-sectionAlt">
  <div class="za-container">
    <div class="za-sectionHead" data-reveal>
      ...
      <p class="za-muted">{{ __('ui.checkout.subtitle') }}</p>
    </div>
    ...
    @if(!empty($missing))
      <div class="lux-card" style="padding:14px; border-left:4px solid #f59e0b; margin-top:14px;">
        ...
        <div class="za-muted" style="margin-top:6px;">{{ __('ui.checkout.missing_hint') }}</div>
      </div>
    @endif
    ...
    <div class="mt-10" style="display:grid; grid-template-columns:1fr; gap:16px;">
      ...
      <div class="lux-card" style="padding:22px;" data-reveal>
        <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap;">
          <div>
            ...
            <div class="za-muted" style="margin-top:6px;">{{ __('ui.checkout.summary_hint') }}</div>
          </div>
        ...
        <div style="margin-top:16px; display:grid; gap:10px; color:#0f172a;">
          ...
          @else
            ...
            <div><strong>{{ __('ui.checkout.hour_price') }}</strong> {{ $pricing['hour_price'] ?? 35 }} {{ $currency }} / {{ __('ui.checkout.per_hour') }}</div>
          @endif
        </div>
        ...
        @if(!$isCustom)
          <div style="display:flex; justify-content:space-between; gap:14px; flex-wrap:wrap; align-items:flex-end;">
            <div>
              ...
              <div style="font-size:22px; font-weight:900; margin-top:4px;">{{ $pack['price'] ?? '—' }}</div>
              <div class="za-muted" style="margin-top:6px; font-size:13px;">{{ __('ui.checkout.total_hint') }}</div>
            </div>
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
              <a class="btn-outline" href="{{ $backUrl }}">{{ __('ui.back') }}</a>
              <form method="POST" action="{{ route('regimes.saudi.submit') }}" style="display:inline;">
                @csrf
        ...
        @else
          ...
          <div class="lux-card" style="padding:16px; background:rgba(15,23,42,.03);">
            <div style="display:flex; justify-content:space-between; flex-wrap:wrap; gap:10px;">
              ...
              <div>
                ...
                <div style="font-size:22px; font-weight:900; color:var(--emerald-950);">{{ $money($totalCents) }}</div>
              </div>
            </div>
          ...
          <div style="margin-top:14px; display:flex; gap:10px; flex-wrap:wrap;">
            <a class="btn-outline" href="{{ $backUrl }}">{{ __('ui.back') }}</a>
            <button class="btn-primary" type="button" disabled style="opacity:.7; cursor:not-allowed;">
              {{ __('ui.checkout.pay_cta_soon') }}
            </button>
          </div>
      ...
      @if($isCustom)
        <div class="lux-card" style="padding:22px;" data-reveal>
          ...
          <div class="za-muted" style="margin-top:6px;">{{ __('ui.checkout.custom_details_subtitle') }}</div>
              ...
              <tbody>
                @foreach($items as $it)
                  ...
                  <tr style="border-top:1px solid rgba(15,23,42,.08);">
                    ...
                    <td style="padding:12px; font-weight:900;">{{ number_format((float)$lineMonthly, 0) }} {{ $currency }}</td>
                  </tr>
                @endforeach
              </tbody>
            </table>
        ...
        <div class="lux-card" style="padding:22px;" data-reveal>
          <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap;">
            <div>
              ...
              <div class="za-muted" style="margin-top:6px;">{{ __('ui.checkout.what_you_get_hint') }}</div>
            </div>
          ...
          @else
            <div class="za-muted" style="margin-top:14px;">{{ __('ui.checkout.no_pack_selected') }}</div>
          @endif
        </div>
      @endif
