...
@php
  ...
  $levelsArr = ($levels ?? collect())->map(function($l){
    ...
    // Saudi: primary-X / college-X / lycee-X
    $group = 'primary';
    if (str_starts_with($slug, 'college')) $group = 'college';
    if (str_starts_with($slug, 'lycee'))   $group = 'lycee';
  ...
  <div class="za-container za-heroInner">
    <div class="za-heroCard" data-reveal>
      ...
      <h1 class="za-heroTitle">
        {{ $trackTitle }} <span style="opacity:.9">&#x1F1F8;&#x1F1E6;</span>
      </h1>
...
{!! json_encode([
  ...
  'groupLabels'    => $groupLabels,
  'startUrl'       => route('regimes.saudi.start'),
  ...
  <div class="za-container" style="position:relative; z-index:2;">
    ...
    <div class="lux-card" style="padding:22px; margin-top:18px;">
      ...
      <div style="margin-top:18px;" x-show="levelId && levelHasBranches()">
        ...
        <div class="za-muted" style="margin-top:6px;">{{ __('ui.branch_hint') ?? 'Choisissez la branche.' }}</div>
        ...
        <div style="margin-top:12px; display:flex; gap:10px; flex-wrap:wrap;">
          ...
          <div x-show="levelId && filteredBranches.length === 0" class="za-muted" style="margin-top:8px;">
            {{ __('ui.no_branches') ?? 'Aucune branche.' }}
          </div>
        </div>
      </div>
      ...
      <div class="za-tracksGrid" style="margin-top:18px;">
        @foreach($packs as $i => $p)
          @php
            ...
            $convDisplay = '';
            if ($showConversion && isset($p['price_amount'])) {
              $converted   = round($p['price_amount'] * $convRate);
              $convDisplay = '≈ ' . $converted . ' ' . $visitorSymbol;
            }
          @endphp
          ...
          >
            <div class="za-trackBody">
              ...
              @if($showConversion && $convDisplay)
                <p class="za-muted" style="font-size:.8rem; margin-top:2px;">{{ $convDisplay }}</p>
              @endif
      ...
      <div class="lux-card" style="padding:22px; margin-top:18px;" x-show="packKey === 'custom'" x-transition.opacity>
        <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap; align-items:flex-start;">
          <div>
            ...
            <p class="za-muted" style="margin-top:6px;">{{ __('ui.regimes.custom_subtitle') }}</p>
          </div>
          <div class="pill">
        ...
        <div class="za-muted" style="margin-top:10px;">{{ __('ui.regimes.custom_example') }}</div>
          ...
          <template x-for="(it, idx) in items" :key="it.id">
            <div class="lux-card" style="padding:14px;">
              ...
              <div style="margin-top:10px; display:flex; justify-content:space-between; flex-wrap:wrap; gap:10px;">
                ...
                <div class="za-muted">
                  (<span x-text="pricePerHour(it)"></span> <span x-text="currencySymbol"></span>
                  × <span x-text="it.duration_h"></span>h
                  × <span x-text="it.sessions_per_month"></span> {{ __('ui.regimes.sessions') }})
                </div>
              </div>
            </div>
          </template>
...
function regimeSelector(cfg){
  return {
    ...
    currencySymbol: cfg.currencySymbol || 'SAR',
    ...
    modePrices: cfg.modePrices || { individual:60, duo:46, group:36 },
    ...
    formatMoney(v){
      const n = Number(v || 0);
      return n.toFixed(0) + ' ' + this.currencySymbol;
    },
    ...
    itemLevelHasBranches(it){ return this.branchesFor(it).length > 0; },
    ...
    pickGroup(g){
      this.group = g; this.levelId = null; this.branchId = null; this.subjectId = null;
      if(this.packKey !== 'custom') this.packKey = null;
      this.hideToast();
    },

    pickLevel(id){
      this.levelId = id; this.branchId = null; this.subjectId = null;
      if(this.packKey !== 'custom') this.packKey = null;
      this.hideToast();
    },

    pickBranch(id){
      this.branchId = id; this.subjectId = null;
      if(this.packKey !== 'custom') this.packKey = null;
      this.hideToast();
    },

    pickSubject(id){ this.subjectId = id; this.hideToast(); },
    ...
    packCardClass(key){
      return [ this.packIsLocked(key) ? 'is-disabled' : 'is-ready', this.packKey === key ? 'is-selected' : '' ].join(' ');
    },
    ...
    pricePerHour(it){
      ...
      return Number(this.modePrices?.[mode] ?? 36);
    },

    newItem(payload = {}){
      return Object.assign({
        id: (window.crypto?.randomUUID?.() || (Date.now() + '-' + Math.random().toString(16).slice(2))),
        student_name: '', level_id: null, branch_id: null, subject_id: null,
        mode: 'group', duration_h: 1, sessions_per_month: 4,
      }, payload);
    },
    ...
    addItemFromCurrent(){
      this.items.push(this.newItem({ level_id: this.levelId ?? null, branch_id: this.branchId ?? null, subject_id: this.subjectId ?? null }));
    },
    ...
    subtotalMonthly(){ return this.items.reduce((sum, it) => sum + this.lineMonthly(it), 0); },

    monthsForPlan(){ if(this.plan === 'yearly') return 12; if(this.plan === 'quarterly') return 3; return 1; },

    discountForPlan(){ return Number(this.discounts?.[this.plan] ?? 0); },
    ...
    canProceed(){
      if(this.packKey === 'custom') return this.customLinesComplete();
...
.za-packCard.is-selected{
  outline: 3px solid var(--emerald-600, #059669);
  outline-offset: 2px;
}
...
.za-packCard.is-highlight{
  border-color: rgba(234,179,8,.4);
  background: linear-gradient(135deg, rgba(234,179,8,.04), rgba(16,185,129,.04));
}
.za-packHead{
  display: flex;
...
.za-packHead.is-custom .za-trackName::before{
  content: '✦ ';
